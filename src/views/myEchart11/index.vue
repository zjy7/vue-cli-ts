...
function processElectriFence(response){
  let obj = {
  ...
  response.data.map((item,index)=>{
    ...
    obj.features[index].geometry.coordinates[0][0] = []
    item.electricFences.map((i1,j1)=>{
      obj.features[index].geometry.coordinates[0][0].push([Number(i1.lon),Number(i1.lat)])
    })
  })
  return obj
}
const ROOT_JIAXING_TRANS = processElectriFence(ROOT_JIAXING)
        ...
        chart.on('click', function (params) {
          ...
          if (opt.goDown && params.name !== name[idx]) {
                if (cityMap[params.name]) {
                  if(params.name === '嘉兴市'){
                      echarts.registerMap(params.name, ROOT_JIAXING_TRANS)
